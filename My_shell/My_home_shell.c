...
char **Parsing(char *str, int *size) { //обработка строки
    ...
    ans = (char **) realloc (ans, sizeof(char *) * BB); //выделили пам€ть под список лексем
    while (1) {
        ...
        if (count != 0) {
            ...
            ans[num] = s;  // добавл€ем слово в список
            num++;
            if (num % BB == 0) 
        ...
        else if ((str[i] =='|') || (str[i] == '>') || (str[i] == '&')) {  // обрабатываем повтор€ющиес€ спец символы
            if (str[i + 1] == str[i]) {
...
int Conv(char **arg, int begin, int end) {
    ...
    int name_fl, status;
    int i = begin, fl = 0;
    int pid, fd[2], fd_in = 0, num = 0;   // fd[0] - дл€ чтени€, fd[1] - дл€ записи
    ...
    while (i < end) {
        ...
        vec[num] = NULL; // признак окончани€ вектора
        pipe(fd);  //создаем канал
        if ((pid = fork()) == 0) { //сын
            dup2(fd_in, 0);  // перенаправл€ем ввод (теперь из fd_in)
            if ((fl == 0) && (fd_change_in != -2)) {
                dup2(fd_change_in, 0); // перенаправл€ем ввод 
            }
            if (i != end) {
                dup2(fd[1], 1); // перенаправл€ем вывод (в канал)
            }
            else if ((i == end) && (fd_change_out != -2)) {
                //printf("HERE\n");
                dup2(fd_change_out, 1); // перенаправл€ем вывод 
            }
            close(fd[0]);   
            if (strcmp(name, "cp") == 0) {
                printf("my command.\n");
		...
		exit(0);
                ...
                execvp(name, vec);  //им€ в текущей директории
...
void Redirection(char *how, char *to) { //анализ перенаправлени€
    //printf("\n");
    if (strcmp(how, "<") == 0) {
        //printf("ѕеренаправление ввода в файл %s\n", to);
        fd_change_in = open(to, O_RDONLY, 0666); 
    }
    else if (strcmp(how, ">") == 0) {
        //printf("ѕеренаправление вывода в файл %s, содержимое отбрасываетс€\n", to);
        fd_change_out = open(to, O_WRONLY | O_CREAT | O_TRUNC, 0666);
    }
    else  if (strcmp(how, ">>") == 0) {
        //printf("ѕеренаправление вывода в файл %s, содержимое сохран€етс€\n", to);
...
int Shell_Com(char **arg, int begin, int end) { 
    ...
    for (int i = begin; i < end; ++i) {
        if ((strcmp(arg[i], "<") == 0) || (strcmp(arg[i], ">") == 0) || (strcmp(arg[i], ">>") == 0)) { // смотрим, есть ли перенаправлени€
...
void Shell_Command(char **arg, int begin, int end) { 
    ...
    for (int i = begin; i < end; ++i) {
        if (strcmp(arg[i], ";") == 0) {
            ...
            //printf(" оманда с условным выполнением (;): ");
            for (int j = begin; j < i; j++) {
                s = arg[j];
                //printf("%s ", s); 
            }
